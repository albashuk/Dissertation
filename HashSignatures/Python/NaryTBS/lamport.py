...
class Lamport(OTS):
    ...
    class SecKey(OTS.SecKey):
        ...
        def copy(self):
            return Lamport.SecKey([[hash_obj.copy() for hash_obj in row] for row in self.VALUES])
    ...
    class PubKey(OTS.PubKey):
        ...
        def copy(self):
            return Lamport.PubKey([[hash_obj.copy() for hash_obj in row] for row in self.VALUES], self.HASH.copy())
    ...
    class Sign(OTS.Sign):
        ...
        def copy(self):
            return Lamport.Sign([hash_obj.copy() for hash_obj in self.VALUES])
